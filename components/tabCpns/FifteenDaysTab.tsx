        ...
        <ScrollView style={styles.container}>
            <View style={styles.card}>
                <View style={styles.headerRow}>
                    ...
                    <Text style={styles.areaText}>{location?.name} <Feather name="chevron-right" size={16} color="#aaa" /></Text>
                </View>
                    ...
                    return (
                        ...
                        >
                            <View style={styles.dateContainer}>
                                <Text style={[styles.dateText, selectedDateIndex === index && styles.selectedText]}>{`${month}/${day}`}</Text>
                                <Text style={[styles.weekdayText, selectedDateIndex === index && styles.selectedText]}>{getWeekday(item.date)}</Text>
                            </View>
                            <View style={styles.weatherIconContainer}>
                                ...
                                />
                                {rainProb > 0 ? (
                                    <Text style={[styles.rainProbText, selectedDateIndex === index && styles.selectedText]}>{rainProb}%</Text>
                                ) : (
                                    <Text style={[styles.rainProbText, { opacity: 0 }]}>0%</Text>
                                )}
                            </View>
                            <View style={styles.tempContainer}>
                                <Text style={[styles.tempText, selectedDateIndex === index && styles.selectedText]}>{item.low}</Text>
                                <Text style={[styles.tempText, selectedDateIndex === index && styles.selectedText, { marginLeft: 8 }]}>{item.high}°</Text>
                            </View>
                        </TouchableOpacity>
                    );
                })}
