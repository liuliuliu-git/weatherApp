...
export default function DailyTab() {
  ...
  const todayWeather = useMemo(
    () => (weatherDaily ? weatherDaily[dateIndex] : null),
    [weatherDaily, dateIndex]
  );
  const todaySun = useMemo(
    () => (sunData ? sunData[dateIndex] : null),
    [sunData, dateIndex]
  );
  // 白天/夜间数据
  const weatherText = todayWeather
    ? dayType === 0
      ? todayWeather.text_day
      : todayWeather.text_night
    : "--";
  ...
  const weatherIcon = (
    <View
      style={{
        ...
        backgroundColor:
          dayType === 0
            ? "rgba(74, 144, 226, 0.15)" // 白天
            : "rgba(25, 55, 109, 0.15)", // 夜晚
        alignItems: "center",
    ...
    >
      {/* <Image
                source={{
                    uri: getWeatherIconUri(Number(dayType === 0 ? todayWeather?.code_day : todayWeather?.code_night), "light")
                }}
                style={{
                    width: 40,
                    height: 40,
  ...
  const rightArrowColor =
    weatherDaily && dateIndex === weatherDaily.length - 1 ? "#ccc" : "#4a90e2";
        ...
        <View style={styles.headerRow}>
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                setSelectedDateIndex(
                  weatherDaily
                    ? Math.min(weatherDaily.length - 1, dateIndex + 1)
                    : dateIndex
                )
              }
              style={styles.arrowBtn}
          ...
          <TouchableOpacity onPress={() => router.push("/search")}>
            <Text style={styles.areaText}>
              {area} <Feather name="chevron-right" size={16} color="#aaa" />
            </Text>
          </TouchableOpacity>
        </View>
        {/* 白天/夜间切换 */}
        <View style={styles.dayNightRow}>
          {DAY_NIGHT.map((label, idx) => (
            <TouchableOpacity
              ...
              style={[
                styles.dayNightBtn,
                dayType === idx && styles.dayNightBtnActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.dayNightText,
                  dayType === idx && styles.dayNightTextActive,
                ]}
              >
                {label}
              </Text>
